...
void KPPMix::computeMixingCoefficients(
   ...
   // Diagnostic: Print non-local flux profile for a sample cell
   {
      static I4 DiagnosticCount = 0;
      if (DiagnosticCount < 3) {
         DiagnosticCount++;
         const I4 SampleCell = 0;
         auto NonLocalFluxH  = createHostMirrorCopy(VertNonLocalFlux);
         auto BoundaryDepthH = createHostMirrorCopy(BoundaryLayerDepth);
         auto ZIntH          = createHostMirrorCopy(VCoord->GeomZInterface);
         auto MinLayerH      = createHostMirrorCopy(VCoord->MinLayerCell);
         auto MaxLayerH      = createHostMirrorCopy(VCoord->MaxLayerCell);

         const Real h_obl = BoundaryDepthH(SampleCell);
         const I4 KMin    = MinLayerH(SampleCell);
         const I4 KMax    = MaxLayerH(SampleCell);

         std::cout << "DEBUG: Non-local flux profile for cell=" << SampleCell
                   << " h_obl=" << h_obl << " m" << std::endl;

         // Compute numerical integral of G(sigma) and (dG/dsigma)
         Real IntegralG    = 0.0;
         Real IntegralDivG = 0.0;
         Real MaxAbsG      = 0.0;

         for (I4 k = KMin; k <= KMax; ++k) {
            const Real z_depth  = Kokkos::abs(ZIntH(SampleCell, k));
            const Real sigma_mu = (h_obl > 0.0) ? z_depth / h_obl : 0.0;
            const Real g_flux   = NonLocalFluxH(SampleCell, k);
            MaxAbsG             = Kokkos::max(MaxAbsG, Kokkos::abs(g_flux));

            if (k == KMin + 3) {
               std::cout << "  k=" << k << ": z=" << z_depth
                         << " m, sigma_mu=" << sigma_mu
                         << ", FluxProfile=" << g_flux << std::endl;
            }
         }

         // Compute divergence integral (flux tendency integral)
         for (I4 k = KMin + 1; k <= KMax; ++k) {
            const Real flux_diff =
                NonLocalFluxH(SampleCell, k) - NonLocalFluxH(SampleCell, k + 1);
            IntegralDivG += flux_diff;
         }

         std::cout << "  max|flux|=" << MaxAbsG
                   << ", integral_div(G)=" << IntegralDivG << std::endl;
      }
   }
