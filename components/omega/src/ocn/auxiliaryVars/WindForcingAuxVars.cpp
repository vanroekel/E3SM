...
WindForcingAuxVars::WindForcingAuxVars(const std::string &AuxStateSuffix,
    ...
    : NormalStressEdge("NormalStressEdge" + AuxStateSuffix, Mesh->NEdgesSize),
      ...
      SalinityPistonVelocity("SalinityPistonVelocity" + AuxStateSuffix,
                             Mesh->NCellsSize),
      TemperatureSurfaceRestoringValue("TemperatureSurfaceRestoringValue" +
                                           AuxStateSuffix,
                                       Mesh->NCellsSize),
      SalinitySurfaceRestoringValue(
          "SalinitySurfaceRestoringValue" + AuxStateSuffix, Mesh->NCellsSize),
      TemperatureInteriorRestoringRate(
          "TemperatureInteriorRestoringRate" + AuxStateSuffix, Mesh->NCellsSize,
          VCoord ? VCoord->NVertLayers : 1),
      SalinityInteriorRestoringRate(
          "SalinityInteriorRestoringRate" + AuxStateSuffix, Mesh->NCellsSize,
          VCoord ? VCoord->NVertLayers : 1),
      ...
      SalinityInteriorRestoringValue(
          "SalinityInteriorRestoringValue" + AuxStateSuffix, Mesh->NCellsSize,
          VCoord ? VCoord->NVertLayers : 1),
      ...
      ShortWaveHeatFlux("ShortWaveHeatFlux" + AuxStateSuffix, Mesh->NCellsSize),
...
void WindForcingAuxVars::registerFields(
   ...
   ZonalStressCellField->addMetadata("InputName",
                                     std::string("windStressZonal"));
   ...
   auto LatentHeatFluxField = Field::create(
       LatentHeatFlux.label(), "latent heat flux", "W m^{-2}", "",
   ...
   auto SensibleHeatFluxField = Field::create(
       SensibleHeatFlux.label(), "sensible heat flux", "W m^{-2}", "",
       std::numeric_limits<Real>::min(), std::numeric_limits<Real>::max(),
       FillValue, NDims, DimNames);
   ...
   auto ShortWaveHeatFluxField = Field::create(
       ShortWaveHeatFlux.label(), "short-wave heat flux", "W m^{-2}", "",
       std::numeric_limits<Real>::min(), std::numeric_limits<Real>::max(),
       FillValue, NDims, DimNames);
   ...
   auto EvaporationFluxField = Field::create(
       EvaporationFlux.label(), "evaporation freshwater flux",
   ...
   auto RainFluxField = Field::create(
       RainFlux.label(), "rain freshwater flux", "kg m^{-2} s^{-1}", "",
       std::numeric_limits<Real>::min(), std::numeric_limits<Real>::max(),
       FillValue, NDims, DimNames);
   RainFluxField->addMetadata("InputName", std::string("rainFlux"));

   auto RiverRunoffFluxField = Field::create(
       RiverRunoffFlux.label(), "river runoff freshwater flux",
       "kg m^{-2} s^{-1}", "", std::numeric_limits<Real>::min(),
       std::numeric_limits<Real>::max(), FillValue, NDims, DimNames);
   ...
   auto IceRunoffFluxField = Field::create(
       IceRunoffFlux.label(), "ice runoff freshwater flux", "kg m^{-2} s^{-1}",
       "", std::numeric_limits<Real>::min(), std::numeric_limits<Real>::max(),
       FillValue, NDims, DimNames);
   IceRunoffFluxField->addMetadata("InputName", std::string("iceRunoffFlux"));

   auto SubglacialRunoffFluxField = Field::create(
       SubglacialRunoffFlux.label(), "subglacial runoff freshwater flux",
       "kg m^{-2} s^{-1}", "", std::numeric_limits<Real>::min(),
       std::numeric_limits<Real>::max(), FillValue, NDims, DimNames);
   SubglacialRunoffFluxField->addMetadata("InputName",
                                          std::string("subglacialRunoffFlux"));

   auto IcebergFreshWaterFluxField = Field::create(
       IcebergFreshWaterFlux.label(), "iceberg freshwater flux",
       "kg m^{-2} s^{-1}", "", std::numeric_limits<Real>::min(),
       std::numeric_limits<Real>::max(), FillValue, NDims, DimNames);
