...
void testStabilityFunctions() {
   ...
   parallelReduce(
       ...
       KOKKOS_LAMBDA(int ITest, int &ErrorCount) {
          const Real Transition =
              ITest == 0 ? KPP::ZETA_M : KPP::ZETA_S;
          const Real Epsilon = 1.0e-6_Real;
          const Real Above = ITest == 0 ? KPP::KPPProfileM2(Transition + Epsilon)
                                        : KPP::KPPProfileS2(Transition + Epsilon);
          const Real Below = ITest == 0 ? KPP::KPPProfileM2(Transition - Epsilon)
                                        : KPP::KPPProfileS2(Transition - Epsilon);
   ...
   parallelReduce(
       ...
       KOKKOS_LAMBDA(int ITest, int &ErrorCount) {
          ...
          if (!isApprox(KPP::KPPProfileMatched(Sigma, ShapeAtBase),
             ...
             ++ErrorCount;
          if (!isApprox(KPP::KPPProfileMatched(Sigma, 0.0_Real),
                        ExpectedSimple, RTol, ATol))
             ++ErrorCount;
          if (!isApprox(KPP::KPPHu(Sigma), KPP::HUON * (1.0_Real + Sigma),
                        RTol, ATol))
             ++ErrorCount;
       },
       NumErrors);
...
void testLangmuirFunctions() {
   ...
   parallelReduce(
       ...
       KOKKOS_LAMBDA(int ITest, int &ErrorCount) {
          ...
          const Real StokesClamped =
              Kokkos::fmax(1.0e-8_Real, ExpectedStokes);
          const Real ExpectedLa = Kokkos::sqrt(UStarClamped / StokesClamped);
          const Real LaInv =
              1.0_Real / Kokkos::fmax(0.5_Real, ExpectedLa);
          const Real ExpectedEnhancement = Kokkos::fmin(
              2.0_Real, Kokkos::fmax(1.0_Real,
                                     Kokkos::sqrt(1.0_Real +
                                                  0.5_Real * LaInv * LaInv)));
   ...
   parallelReduce(
       ...
       KOKKOS_LAMBDA(int ITest, int &ErrorCount) {
          const Real IceFraction = ITest == 0   ? 0.0_Real
                                   ...
                                   : ITest == 2
                                       ? KPP::ICE_SUPPRESSION_THRESHOLD +
                                             0.01_Real
                                       : 0.0_Real;
          const I4 LandIceMask = ITest == 3 ? 1 : 0;
          const bool ExpectedSuppression =
              LandIceMask != 0 ||
              IceFraction > KPP::ICE_SUPPRESSION_THRESHOLD;
          ...
          if (!isApprox(KPP::ConstrainOBLDepth(InputDepth, 4.0_Real,
...
void testTurbulentVelocityScale() {
   ...
   parallelReduce(
       ...
       KOKKOS_LAMBDA(int ITest, int &ErrorCount) {
          ...
          const Real Momentum =
              UStarClamped * UStarClamped * UStarClamped;
          const Real Buoyancy =
              0.35_Real * Kokkos::fmax(0.0_Real, -B0) * HClamped;
          const Real Expected = Kokkos::pow(Momentum + Buoyancy,
                                            1.0_Real / 3.0_Real);
          const Real Actual =
              KPP::ComputeTurbulentVelocityScale(UStar, B0, H);
...
void setCoefficientTestGeometry() {
   ...
   OMEGA_SCOPE(IndexBoundaryLayerDepth,
               KPPInstance->IndexBoundaryLayerDepth);

   parallelFor(
       "KPPMixTest-SetGeometry", {Mesh->NCellsAll},
       KOKKOS_LAMBDA(I4 ICell) {
          ...
          for (I4 K = 0; K <= NVertLayers; ++K) {
             ...
             if (K < NVertLayers) {
                GeomZMid(ICell, K) = -LayerThickness * (K + 0.5_Real);
             }
          }
       });
}
...
void testWindOnlyCoefficients() {
   ...
   for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
      ...
      if (!isApprox(KPP::KPPProfileM1(Sigma), Shape, RTol, ATol) ||
          VertDiffH(ICell, 0) != 0.0_Real ||
          VertViscH(ICell, 0) != 0.0_Real ||
          VertDiffH(ICell, 4) != 0.0_Real ||
          VertViscH(ICell, 4) != 0.0_Real ||
          VertDiffH(ICell, 5) != 0.0_Real ||
          NonLocalH(ICell, 5) != 0.0_Real) {
         ++NumErrors;
      }
   }
...
void testConvectionOnlyCoefficients() {
   ...
   const Real WM = VonKar *
                   Kokkos::pow(KPP::C_MO_M * SigmaLoc * TestOBLDepth *
                                   VonKar * 1.0e-7_Real,
                               1.0_Real / 3.0_Real);
   const Real WS = VonKar *
                   Kokkos::pow(KPP::C_MO_S * SigmaLoc * TestOBLDepth *
                                   VonKar * 1.0e-7_Real,
                               1.0_Real / 3.0_Real);
...
void testNonLocalProfileModes() {
   ...
   void testMatchBothInteriorCoefficients() {
      ...
      const Real TurbVel           = VonKar * 0.02_Real;
      const Real ExpectedDiffMid =
         TestOBLDepth * TurbVel * SimpleShape +
         SmoothAtSigma * ExpectedInteriorDiff;
      const Real ExpectedViscMid =
         TestOBLDepth * TurbVel * SimpleShape +
   ...
   void testEnhancedDiffusion() {
      ...
      const Real OutsideProfile =
         ...
         KPP::KPPProfileS1(OutsideSigma);
      const Real ExpectedOutside =
         OutsideDelta * (1.0_Real - OutsideDelta) *
      ...
      const Real TargetProfile =
         InsideOBLDepth * VonKar * 0.02_Real *
         KPP::KPPProfileS1(TargetSigma);
      const Real ExpectedInside =
         InsideDelta *
         (OneMinusInsideDelta * OneMinusInsideDelta * KtupProfile +
         InsideDelta * InsideDelta * TargetProfile);
      const Real ExpectedInsideNonLocal =
         nonLocalNormalization() * KPP::KPPProfileG(TargetSigma) *
      ...
      for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
        if (!isApprox(VertDiffH(ICell, 3), ExpectedInside, RTol, ATol) ||
           ...
           !isApprox(NonLocalH(ICell, 3), ExpectedInsideNonLocal, RTol,
                  ATol)) {
          ++NumErrors;
        }
      }

        Array2DReal InteriorDiff("KPPMixTest-EnhancedInteriorDiff",
                           Mesh->NCellsSize, NVertLayers + 1);
        Array2DReal InteriorVisc("KPPMixTest-EnhancedInteriorVisc",
                           Mesh->NCellsSize, NVertLayers + 1);
      ...
      const Real InteriorKtupSigma = -35.0_Real / TestOBLDepth;
        ...
        const Real DiffKtup = TestOBLDepth * VonKar * 0.02_Real *
                        KPP::KPPProfileMatched(InteriorKtupSigma,
                                          DiffMatchShape);
        const Real ViscKtup = TestOBLDepth * VonKar * 0.02_Real *
                        KPP::KPPProfileMatched(InteriorKtupSigma,
                                          ViscMatchShape);
        ...
        for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
          if (!isApprox(VertDiffH(ICell, 4), ExpectedInteriorEnhancedDiff,
                     RTol, ATol) ||
             !isApprox(VertViscH(ICell, 4), ExpectedInteriorEnhancedVisc,
                     RTol, ATol)) {
            ++NumErrors;
          }
        }
      ...
      NonLocalH = createHostMirrorCopy(KPPInstance->VertNonLocalFlux);
      for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
         if (VertDiffH(ICell, 3) != 0.0_Real ||
             VertViscH(ICell, 3) != 0.0_Real ||
...
void testCoefficientVerticalDomainEdges() {
   ...
   Array2DReal Density("KPPMixTest-EdgeDensity", Mesh->NCellsSize,
                       NVertLayers);
   ...
   int NumErrors = 0;
   for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
      if (VertDiffH(ICell, 0) != 0.0_Real ||
          VertDiffH(ICell, 1) != 0.0_Real ||
...
void testBoundaryLayerDepth() {
   ...
   Array2DReal Density("KPPMixTest-BLDDensity", Mesh->NCellsSize,
                       NVertLayers);
   Array2DReal NormalVelocity("KPPMixTest-BLDNormalVelocity",
   ...
   constexpr Real RiScaling =
       1.0_Real - 0.5_Real * KPP::SURFACE_LAYER_EXTENT;
   constexpr Real TestN = 1.0_Real;
   const Real UnresolvedShearConstant =
       Kokkos::sqrt(0.2_Real /
                    (KPP::C_MO_S * KPP::SURFACE_LAYER_EXTENT)) /
       (VonKar * VonKar);
   ...
   parallelFor(
       ...
       KOKKOS_LAMBDA(I4 ICell, I4 K) {
          ...
          const Real Vt2 =
              1.7_Real * UnresolvedShearConstant * ZCenter * TestN *
   ...
   const auto BuoyancyJumpH =
       createHostMirrorCopy(KPPInstance->BuoyancyJump);
   ...
   const Real ExpectedBLD =
       ZAbove + (-Slope + Kokkos::sqrt(Discriminant)) /
                    (2.0_Real * Quadratic);
   const Real ExpectedVt2 =
       1.7_Real * UnresolvedShearConstant * 25.0_Real * TestN *
       WindTurbulentScale / 0.25_Real;
   const Real ExpectedDeltaB =
       0.4_Real * ExpectedVt2 / (RiScaling * 25.0_Real);
   ...
   for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
      if (!isApprox(BulkRiH(ICell, 2), RiAbove, BLDRTol, ATol) ||
          ...
          BLDIndexH(ICell) != 2 ||
          !isApprox(BulkShearH(ICell, 3), 1.0e-15_Real, RTol,
                    1.0e-16_Real) ||
   ...
   parallelFor(
       "KPPMixTest-SetLinearRichardsonDensity",
       {Mesh->NCellsAll, NVertLayers}, KOKKOS_LAMBDA(I4 ICell, I4 K) {
   ...
   parallelFor(
       "KPPMixTest-SetResolvedShear",
       {Mesh->NEdgesAll, NVertLayers}, KOKKOS_LAMBDA(I4 IEdge, I4 K) {
   ...
   parallelFor(
       "KPPMixTest-SetShallowCrossingDensity",
...
void testBoundaryLayerNonuniformThickness() {
   ...
   deepCopy(IceFraction, 0.0_Real);
      parallelFor(
         "KPPMixTest-SetNonuniformVelocity",
         {Mesh->NEdgesAll, NVertLayers}, KOKKOS_LAMBDA(I4 IEdge, I4 K) {
   ...
   parallelFor(
       ...
       KOKKOS_LAMBDA(I4 ICell) {
          ...
          for (I4 K = 0; K <= NVertLayers; ++K) {
           ...
           const Real Vt2Layer1 =
              1.7_Real * UnresolvedShearConstant * 2.0_Real * TestN *
              WindTurbulentScale / 0.25_Real;
           const Real Vt2Layer2 =
              1.7_Real * UnresolvedShearConstant * 6.5_Real * TestN *
              WindTurbulentScale / 0.25_Real;
           const Real Vt2Layer3 =
              1.7_Real * UnresolvedShearConstant * 17.5_Real * TestN *
              WindTurbulentScale / 0.25_Real;
           const Real DeltaRho1 =
              0.05_Real * (Shear1 + Vt2Layer1) * RhoSw /
              (RiScaling * Gravity * 2.0_Real);
           const Real DeltaRho2 =
              0.10_Real * Vt2Layer2 * RhoSw /
              (RiScaling * Gravity * 6.5_Real);
           const Real DeltaRho3 =
              0.40_Real * (Shear3 + Vt2Layer3) * RhoSw /
          ...
          const Real WeightedSurfaceDensity =
              (Density(ICell, 0) + 2.0_Real * Density(ICell, 1)) /
              3.0_Real;
   ...
   const Real ExpectedDeltaB =
         0.40_Real *
         (0.25_Real +
         1.7_Real * UnresolvedShearConstant * ZBelow * TestN *
            WindTurbulentScale / 0.25_Real) /
       (RiScaling * ZBelow);
   ...
   for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
      if (!isApprox(BulkRiH(ICell, 2), RiPrevious, BLDRTol, ATol) ||
          ...
          !isApprox(BuoyancyJumpH(ICell, 4), ExpectedDeltaB, BLDRTol,
                    ATol) ||
          !isApprox(BLDH(ICell), ExpectedBLD, BLDRTol, ATol) ||
          BLDIndexH(ICell) != 3) {
         ++NumErrors;
...
void testBoundaryLayerEdgeFallbacks() {
   ...
   deepCopy(IceFraction, 0.0_Real);
   parallelFor(
       "KPPMixTest-SetEdgeFallbackVelocity",
       {Mesh->NEdgesAll, NVertLayers}, KOKKOS_LAMBDA(I4 IEdge, I4 K) {
          NormalVelocity(IEdge, K)     = 0.1_Real * K;
          TangentialVelocity(IEdge, K) = 0.2_Real * K;
       });
   ...
   for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
      if (!isApprox(BulkShearH(ICell, 3), 1.0e-15_Real, RTol,
                    1.0e-16_Real)) {
         ++NumErrors;
      }
   }
   ...
   void testBoundaryLayerLangmuir() {
      ...
      Array2DReal BVF("KPPMixTest-LangmuirBVF", Mesh->NCellsSize,
                  NVertLayers + 1);
      ...
      parallelFor(
         ...
         KOKKOS_LAMBDA(I4 ICell, I4 K) {
           ...
           const Real TargetRi = K == 0 ? 0.0_Real : (K == 1 ? 0.1_Real
                                                : 0.26_Real);
           const Real DeltaRho =
              TargetRi * Vt2 * RhoSw / (RiScaling * Gravity * ZCenter);
           Density(ICell, K) = RhoSw + DeltaRho;
      ...
      KPPInstance->computeOBLDepth(Density, NormalVelocity, TangentialVelocity,
                           UStar, B0, BVF, IceFraction, Wind);
      Array1DReal DisabledBLD("KPPMixTest-LangmuirDisabledBLD",
                              Mesh->NCellsAll);
      Array2DReal DisabledRi("KPPMixTest-LangmuirDisabledRi",
                             Mesh->NCellsAll, NVertLayers + 1);
      Array2DReal DisabledVt2("KPPMixTest-LangmuirDisabledVt2",
                              Mesh->NCellsAll, NVertLayers + 1);
      ...
      const auto DisabledBLDH =
         createHostMirrorCopy(DisabledBLD);
      ...
      KPPInstance->computeOBLDepth(Density, NormalVelocity, TangentialVelocity,
                           UStar, B0, BVF, IceFraction, Wind);
      Array1DReal EnabledBLD("KPPMixTest-LangmuirEnabledBLD",
                             Mesh->NCellsAll);
      Array2DReal EnabledRi("KPPMixTest-LangmuirEnabledRi", Mesh->NCellsAll,
                            NVertLayers + 1);
      Array2DReal EnabledVt2("KPPMixTest-LangmuirEnabledVt2",
                             Mesh->NCellsAll, NVertLayers + 1);
      ...
      const auto EnabledBLDH =
         createHostMirrorCopy(EnabledBLD);
      ...
      const Real ExpectedDisabledVt2 =
         1.7_Real * UnresolvedShearConstant * ZCenter * TestN *
         DisabledWTurb / 0.25_Real;
      const Real ExpectedEnabledVt2 =
         1.7_Real * UnresolvedShearConstant * ZCenter * TestN * EnabledWTurb /
         0.25_Real;
      ...
      for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
         if (!isApprox(DisabledRiH(ICell, 3), 0.26_Real, BLDRTol, ATol) ||
             !isApprox(EnabledRiH(ICell, 3), ExpectedEnabledRi, BLDRTol,
                       ATol)) {
            ++RiErrors;
         }
         if (!isApprox(DisabledVt2H(ICell, 3), ExpectedDisabledVt2, RTol,
                       ATol) ||
             !isApprox(EnabledVt2H(ICell, 3), ExpectedEnabledVt2, RTol,
                       ATol)) {
            ++Vt2Errors;
         }
         ...
         if (!isApprox(SuppressedRiH(ICell, 3), DisabledRiH(ICell, 3), RTol,
                       ATol) ||
             !isApprox(SuppressedBLDH(ICell), DisabledBLDH(ICell), RTol,
                       ATol)) {
            ++SuppressionErrors;
         }
      }
   ...
   void testBoundaryLayerSmoothing() {
      ...
      parallelFor(
          ...
          KOKKOS_LAMBDA(I4 ICell, I4 K) {
             ...
             if (ICell % 2 == 0) {
                ...
                TargetRi = K == 1 ? 0.05_Real : (K == 2 ? 0.1_Real
                                                        : (K >= 3 ? 0.4_Real
                                                                  : 0.0_Real));
             }
             const Real ZCenter = LayerThickness * (K + 0.5_Real);
      ...
      KPPInstance->computeOBLDepth(Density, NormalVelocity, TangentialVelocity,
                                   UStar, B0, BVF, IceFraction, Wind);
        Array1DReal UnsmoothedBLD("KPPMixTest-UnsmoothedBLD",
                           Mesh->NCellsAll);
      ...
      for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
         ...
         for (I4 J = 0; J < Mesh->NEdgesOnCellH(ICell); ++J) {
            ...
            WeightedDepth +=
                2.0_Real * Mesh->AreaCellH(INeighbor) *
                UnsmoothedBLDH(INeighbor);
            AreaSum += 2.0_Real * Mesh->AreaCellH(INeighbor);
            ++ValidNeighbors;
         }
         if (ValidNeighbors > 0) {
            WeightedDepth += UnsmoothedBLDH(ICell) * ValidNeighbors *
                             Mesh->AreaCellH(ICell);
            AreaSum += ValidNeighbors * Mesh->AreaCellH(ICell);
         }
         Real ExpectedDepth =
             AreaSum > 0.0_Real ? WeightedDepth / AreaSum
                                : UnsmoothedBLDH(ICell);
      ...
      void testEnabledFullCall() {
         ...
         Array2DReal BVF("KPPMixTest-FullCallBVF", Mesh->NCellsSize,
                     NVertLayers + 1);
         ...
         Array1DI4 ExpectedBLDIndex("KPPMixTest-ExpectedBLDIndex",
                                    Mesh->NCellsAll);
         Array2DReal ExpectedBulkRi("KPPMixTest-ExpectedBulkRi",
                                    Mesh->NCellsAll, NVertLayers + 1);
         Array2DReal ExpectedVertDiff("KPPMixTest-ExpectedVertDiff",
                                      Mesh->NCellsAll, NVertLayers + 1);
         Array2DReal ExpectedVertVisc("KPPMixTest-ExpectedVertVisc",
                                      Mesh->NCellsAll, NVertLayers + 1);
         Array2DReal ExpectedNonLocal("KPPMixTest-ExpectedNonLocal",
                                      Mesh->NCellsAll, NVertLayers + 1);
         Array2DReal ExpectedTurbVel("KPPMixTest-ExpectedTurbVel",
                                     Mesh->NCellsAll, NVertLayers + 1);
         ...
         const auto ExpectedBLDIndexH =
            createHostMirrorCopy(ExpectedBLDIndex);
         ...
         const auto ActualBulkRiH =
            createHostMirrorCopy(KPPInstance->BulkRichardsonNumber);
         const auto ActualVertDiffH =
            createHostMirrorCopy(KPPInstance->VertDiff);
         const auto ActualVertViscH =
            createHostMirrorCopy(KPPInstance->VertVisc);
         ...
         for (I4 ICell = 0; ICell < Mesh->NCellsAll; ++ICell) {
           ...
           for (I4 K = 0; K <= NVertLayers; ++K) {
             if (!isApprox(ActualBulkRiH(ICell, K), ExpectedBulkRiH(ICell, K),
                        RTol, ATol) ||
                !isApprox(ActualVertDiffH(ICell, K),
                        ExpectedVertDiffH(ICell, K), RTol, ATol) ||
                !isApprox(ActualVertViscH(ICell, K),
                        ExpectedVertViscH(ICell, K), RTol, ATol) ||
                !isApprox(ActualNonLocalH(ICell, K),
...
void testDisabledFullCall() {
   ...
   Array2DReal BVF("KPPMixTest-DisabledBVF", Mesh->NCellsSize,
                   NVertLayers + 1);
...
int main(int argc, char *argv[]) {
   ...
   if (TestGroup == "integration" || TestGroup == "all") {
      ...
      if (TestGroup == "config-gradient" ||
         TestGroup == "config-unsupported") {
        testConfigurationNormalization();
      }
   if (TestGroup != "profiles" && TestGroup != "bld" &&
       TestGroup != "vmix" && TestGroup != "integration" &&
         TestGroup != "config-gradient" &&
         TestGroup != "config-unsupported" &&
       TestGroup != "all") {
