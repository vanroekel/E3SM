...
!=================================================================================================
  ...
  type, public :: elm_interface_th_datatype
     ...
     ! soilstate_vars:
     real(r8), pointer :: soilpsi                               (:,:) =>null()  ! col soil water potential in each soil layer (MPa) (CN)

     ! waterstate_vars:
     real(r8), pointer :: frac_sno_eff                          (:)   =>null()  ! col fraction of ground covered by snow (0 to 1)
     real(r8), pointer :: frac_h2osfc                           (:)   =>null()  ! col fractional area with surface water greater than zero
     real(r8), pointer :: h2osoi_vol                            (:,:) =>null()  ! col volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]  (nlevgrnd)
     real(r8), pointer :: h2osoi_liq                            (:,:) =>null()  ! col liquid water (kg/m2) (-nlevsno+1:nlevgrnd)
     real(r8), pointer :: h2osoi_ice                            (:,:) =>null()  ! col ice lens (kg/m2) (-nlevsno+1:nlevgrnd)
     real(r8), pointer :: h2osfc                                (:)   =>null()  ! col surface water (mm H2O)

     ! temperature_vars:
     real(r8), pointer :: t_soisno                              (:,:) =>null()  ! col soil temperature (Kelvin)  (-nlevsno+1:nlevgrnd)
     real(r8), pointer :: t_grnd                                (:)   =>null()  ! col ground(-air interface averaged) temperature (Kelvin)
     real(r8), pointer :: t_h2osfc                              (:)   =>null()  ! col surface-water temperature [Kelvin]
     real(r8), pointer :: t_nearsurf                            (:)   =>null()  ! col mixed air/veg. temperature near ground surface (for coupling with PFLOTRAN as BC)

     ! canopystate_vars
     integer , pointer :: alt_indx                              (:)   =>null()  ! col current depth of thaw

     ! waterflux_vars:
     real(r8), pointer :: qflx_top_soil                         (:)   =>null()  ! col net water input into soil from top (mm/s)
     real(r8), pointer :: qflx_subl_snow                        (:)   =>null()  ! col sublimation rate from snow pack (mm H2O /s) [+]
     real(r8), pointer :: qflx_evap_soil                        (:)   =>null()  ! col soil surface evaporation (mm H2O/s) (+ = to atm)
     real(r8), pointer :: qflx_evap_snow                        (:)   =>null()  ! col snow surface evaporation (mm H2O/s) (+ = to atm)
     real(r8), pointer :: qflx_evap_h2osfc                      (:)   =>null()  ! col evaporation flux from surface water (mm H2O /s) [+ to atm] ! note: the energy unit of orginal definition incorrect
     real(r8), pointer :: qflx_tran_veg                         (:)   =>null()  ! col vegetation transpiration (mm H2O/s) (+ = to atm)
     real(r8), pointer :: qflx_rootsoil                         (:,:) =>null()  ! col p-aggregated vertically-resolved vegetation/soil water exchange (m H2O/s) (+ = to atm)
     ! new: when coupling with pflotran, evap+tran from CLM is to be as an input, which may be re-adjusted by pflotran flow module.
     real(r8), pointer :: qflx_et_reduced                       (:)   =>null()  ! col net water exchange re-adjusted for mass-balance check (m H2O/s) (+ = to atm)

     real(r8), pointer :: qflx_infl                             (:)   =>null()  ! col infiltration (mm H2O/s)
     real(r8), pointer :: qflx_exfl                             (:)   =>null()  ! col saturate soil excess outflow upwardly (mm H2O/s)
     real(r8), pointer :: qflx_surf                             (:)   =>null()  ! col ground surface runoff/flood/overland flow (mm H2O/s)
     real(r8), pointer :: qflx_h2osfc                           (:)   =>null()  ! col water-covered (pond) surface runoff/flood/overflow (mm H2O/s)
     real(r8), pointer :: qflx_base                             (:)   =>null()  ! col drainage/upflow at bottom (baseflow) (mm H2O/s)
     real(r8), pointer :: qflx_drain                            (:)   =>null()  ! col liquid water losted as drainage (mm H2O/s)
     real(r8), pointer :: qflx_drain_vr                         (:,:) =>null()  ! col vertically-resolved liquid water losted as drainage (mm H2O/s)
     real(r8), pointer :: qflx_lateral                          (:)   =>null()  ! col net liquid water lateral flow (mm H2O/s)
     real(r8), pointer :: qflx_lateral_vr                       (:,:) =>null()  ! col vertically-resolved net liquid water lateral flow (mm H2O/s)

     ! energyflux_vars:
     real(r8), pointer :: htvp                                  (:)  =>null()   ! latent heat of vapor of water (or sublimation) [j/kg]
     real(r8), pointer :: eflx_bot                              (:)  =>null()   ! col heat flux from beneath the soil or ice column (W/m**2)
     real(r8), pointer :: eflx_soil_grnd                        (:)  =>null()   ! col soil heat flux (W/m**2) [+ = into soil]
     real(r8), pointer :: eflx_fgr0_snow                        (:)  =>null()   ! col heat flux from snow bottom to first soil layer (W/m**2) [+ = into soil]
     real(r8), pointer :: eflx_fgr0_h2osfc                      (:)  =>null()   ! col heat flux from surface water bottom to first soil layer (W/m**2) [+ = into soil]
     real(r8), pointer :: eflx_fgr0_soil                        (:)  =>null()   ! col heat flux from near-surface air to first soil layer (W/m**2) [+ = into soil]
     real(r8), pointer :: eflx_rnet_soil                        (:)  =>null()   ! net radiation flux between soil layer 1 and above-air, excluding SH and LE (i.e. radiation form only ) (W/m2) [+ = into soil]

     ! soilhydrology_vars:
     real(r8), pointer :: frost_table                           (:)  =>null()   ! col frost table depth
     real(r8), pointer :: zwt                                   (:)  =>null()   ! col water table depth
     real(r8), pointer :: zwt_perched                           (:)  =>null()   ! col perched water table depth
     real(r8), pointer :: qcharge                               (:)  =>null()   ! col aquifer recharge rate (mm/s)

     ! atm2lnd:
     real(r8), pointer :: forc_pbot                             (:)  =>null()   ! col atm pressure (Pa)
...
!-------------------------------------------------------------------------------------------------
  ...
  subroutine InitAllocate(this, bounds)
    ...
    !soilstate_vars:
    allocate(this%soilpsi           (begc:endc, 1:nlevgrnd))            ; this%soilpsi          (:,:) = spval
