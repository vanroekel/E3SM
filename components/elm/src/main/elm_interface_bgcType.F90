...
!
  ...
  type, public :: elm_interface_bgc_datatype
     ...
     ! decomp_cascade_con
     logical, pointer           :: floating_cn_ratio                (:) => null()  ! TRUE => pool has fixed C:N ratio
     logical, pointer           :: floating_cp_ratio                (:) => null()  ! TRUE => pool has fixed C:P ratio
     character(len=8), pointer  :: decomp_pool_name                 (:) => null()  ! name of pool
     real(r8), pointer          :: initial_cn_ratio                 (:) => null()  ! c:n ratio for initialization of pools
     real(r8), pointer          :: initial_cp_ratio                 (:) => null()  ! c:p ratio for initialization of pools

     ! ch4
     real(r8), pointer  :: finundated                           (:)     => null()  ! col fractional inundated area (excluding dedicated wetland cols)
     real(r8), pointer  :: o2stress_unsat                       (:,:)   => null()  ! col Ratio of oxygen available to that demanded by roots, aerobes, & methanotrophs (nlevsoi)
     real(r8), pointer  :: o2stress_sat                         (:,:)   => null()  ! col Ratio of oxygen available to that demanded by roots, aerobes, & methanotrophs (nlevsoi)
     real(r8), pointer  :: conc_o2_sat                          (:,:)   => null()  ! col O2 conc in each soil layer (mol/m3) (nlevsoi)
     real(r8), pointer  :: conc_o2_unsat                        (:,:)   => null()  ! col O2 conc in each soil layer (mol/m3) (nlevsoi)
     real(r8), pointer  :: o2_decomp_depth_sat                  (:,:)   => null()  ! col O2 consumption during decomposition in each soil layer (nlevsoi) (mol/m3/s)
     real(r8), pointer  :: o2_decomp_depth_unsat                (:,:)   => null()  ! col O2 consumption during decomposition in each soil layer (nlevsoi) (mol/m3/s)

     ! cnstate_vars:
     real(r8) , pointer :: rf_decomp_cascade                    (:,:,:) => null()  ! col respired fraction in decomposition step (frac)
     real(r8) , pointer :: pathfrac_decomp_cascade              (:,:,:) => null()  ! col what fraction of C leaving a given pool passes through a given
     ...
     real(r8), pointer :: smin_nh4sorb_vr                       (:,:)   => null()  ! col (gN/m3) vertically-resolved soil mineral NH4 absorbed
     ...
     real(r8), pointer :: sminp_vr                              (:,:)   => null()  ! col (gP/m3) vertically-resolved soil parimary mineral P


     ! plant NP demand
     real(r8), pointer :: plant_ndemand                         (:)     => null()  ! col N flux required to support initial GPP (gN/m3/s)
     real(r8), pointer :: plant_pdemand                         (:)     => null()  ! col P flux required to support initial GPP (gP/m3/s)
     real(r8), pointer :: plant_ndemand_vr                      (:,:)   => null()  ! col vertically-resolved N flux required to support initial GPP (gN/m3/s)
     real(r8), pointer :: plant_pdemand_vr                      (:,:)  =>  null()  ! col vertically-resolved P flux required to support initial GPP (gP/m3/s)
     ...
     real(r8), pointer :: decomp_npools_sourcesink              (:,:,:) => null()  ! col (gN/m3) change in decomposing n pools
     real(r8), pointer :: decomp_ppools_sourcesink              (:,:,:) => null()  ! col (gP/m3) change in decomposing n pools

     real(r8), pointer :: decomp_cascade_ctransfer_vr           (:,:,:) => null()  ! vertically-resolved C transferred along deomposition cascade (gC/m3/s)
     real(r8), pointer :: decomp_cascade_ntransfer_vr           (:,:,:) => null()  ! col vert-res transfer of N from donor to receiver pool along decomp. cascade (gN/m3/s)
     real(r8), pointer :: decomp_cascade_ptransfer_vr           (:,:,:) => null()  ! col vert-res transfer of P from donor to receiver pool along decomp. cascade (gP/m3/s)
     real(r8), pointer :: decomp_cascade_sminn_flux_vr          (:,:,:) => null()  ! col vert-res mineral N flux for transition along decomposition cascade (gN/m3/s)
     real(r8), pointer :: decomp_cascade_sminp_flux_vr          (:,:,:) => null()  ! col vert-res mineral P flux for transition along decomposition cascade (gP/m3/s)
     real(r8), pointer :: sminn_to_denit_decomp_cascade_vr      (:,:,:) => null()  ! col vertically-resolved denitrification along decomp cascade (gN/m3/s)
     ...
     real(r8), pointer :: gross_nmin_vr                         (:,:)   => null()  ! col vertically-resolved gross rate of N mineralization (gN/m3/s)
     real(r8), pointer :: net_nmin_vr                           (:,:)   => null()  ! col vertically-resolved net rate of N mineralization (gN/m3/s)

     real(r8), pointer :: potential_immob_vr                    (:,:)   => null()  ! col vertically-resolved potential N immobilization (gN/m3/s) at each level
     real(r8), pointer :: actual_immob_vr                       (:,:)   => null()  ! col vertically-resolved actual N immobilization (gN/m3/s) at each level
     real(r8), pointer :: actual_immob_no3_vr                   (:,:)   => null()  ! col vertically-resolved actual immobilization of NO3 (gN/m3/s)
     real(r8), pointer :: actual_immob_nh4_vr                   (:,:)   => null()  ! col vertically-resolved actual immobilization of NH4 (gN/m3/s)
     real(r8), pointer :: supplement_to_sminn_vr                (:,:)   => null()  ! col vertically-resolved supplemental N supply (gN/m3/s)

     real(r8), pointer :: sminn_to_plant_vr                     (:,:)   => null()  ! col vertically-resolved plant uptake of soil mineral N (gN/m3/s)
     real(r8), pointer :: smin_no3_to_plant_vr                  (:,:)   => null()  ! col vertically-resolved plant uptake of soil NO3 (gN/m3/s)
     real(r8), pointer :: smin_nh4_to_plant_vr                  (:,:)   => null()  ! col vertically-resolved plant uptake of soil NH4 (gN/m3/s)

     real(r8), pointer :: potential_immob                       (:)     => null()  ! col vert-int (diagnostic) potential N immobilization (gN/m2/s)
     real(r8), pointer :: actual_immob                          (:)     => null()  ! col vert-int (diagnostic) actual N immobilization (gN/m2/s)
     real(r8), pointer :: sminn_to_plant                        (:)     => null()  ! col vert-int (diagnostic) plant uptake of soil mineral N (gN/m2/s)

     real(r8), pointer :: potential_immob_p_vr                  (:,:)   => null()  ! col vertically-resolved potential P immobilization (gP/m3/s) at each level
     real(r8), pointer :: actual_immob_p_vr                     (:,:)   => null()  ! col vertically-resolved actual P immobilization (gP/m3/s) at each level
     real(r8), pointer :: sminp_to_plant_vr                     (:,:)   => null()  ! col vertically-resolved plant uptake of soil mineral P (gP/m3/s)
     real(r8), pointer :: supplement_to_sminp_vr                (:,:)   => null()  ! col vertically-resolved supplemental P supply (gP/m3/s)
     
     real(r8), pointer :: gross_pmin_vr                         (:,:)   => null()  ! col vertically-resolved gross rate of P mineralization (gP/m3/s)
     real(r8), pointer :: net_pmin_vr                           (:,:)  => null()   ! col vertically-resolved net rate of P mineralization (gP/m3/s)

     real(r8), pointer :: potential_immob_p                     (:)    => null()   ! col vert-int (diagnostic) potential P immobilization (gP/m2/s)
     real(r8), pointer :: actual_immob_p                        (:)    => null()   ! col vert-int (diagnostic) actual P immobilization (gP/m2/s)
     real(r8), pointer :: sminp_to_plant                        (:)    => null()   ! col vert-int (diagnostic) plant uptake of soil mineral P (gP/m2/s)
     
     ! nitrification / denitrification flux:
     real(r8), pointer :: f_nit_vr                              (:,:)  => null()  ! col (gN/m3/s) soil nitrification flux
     real(r8), pointer :: f_denit_vr                            (:,:)  => null()  ! col (gN/m3/s) soil denitrification flux
     real(r8), pointer :: pot_f_nit_vr                          (:,:)  => null()  ! col (gN/m3/s) potential soil nitrification flux
     real(r8), pointer :: pot_f_denit_vr                        (:,:)  => null()  ! col (gN/m3/s) potential soil denitrification flux
     real(r8), pointer :: n2_n2o_ratio_denit_vr                 (:,:)  => null()  ! col ratio of N2 to N2O production by denitrification [gN/gN]
     real(r8), pointer :: f_n2o_denit_vr                        (:,:)  => null()  ! col flux of N2o from denitrification [gN/m^3/s]
     real(r8), pointer :: f_n2o_nit_vr                          (:,:)  => null()  ! col flux of N2o from nitrification [gN/m^3/s]

     real(r8), pointer :: sminn_to_denit_excess_vr              (:,:)  => null()  ! col vertically-resolved denitrification from excess mineral N pool (gN/m3/s)

     ! inorganic P transformation
     real(r8), pointer :: primp_to_labilep_vr                   (:,:)  => null()  ! col (gP/m3/s) flux of P from primary mineral to labile
     real(r8), pointer :: labilep_to_secondp_vr                 (:,:)  => null()  ! col (gP/m3/s) flux of labile P to secondary mineral P
     real(r8), pointer :: secondp_to_labilep_vr                 (:,:)  => null()  ! col (gP/m3/s) flux of the desorption of secondary mineral P to labile P
     real(r8), pointer :: secondp_to_occlp_vr                   (:,:)  => null()  ! col (gP/m3/s) flux of the occlusion of secondary P to occluded P
     ...
     real(r8), pointer :: f_n2o_soil_vr                         (:,:)  => null()  ! col flux of N2o from soil-N processes [gN/m^3/s]
     real(r8), pointer :: f_n2_soil_vr                          (:,:)  => null()  ! col flux of N2 from soil-N processes [gN/m^3/s]
     ...
     ! fpi, fpg
     real(r8) , pointer :: fpi_vr                               (:,:)  => null()  ! col fraction of potential immobilization (no units)
     real(r8) , pointer :: fpi                                  (:)    => null()  ! col fraction of potential immobilization (no units)
     real(r8),  pointer :: fpg                                  (:)    => null()  ! col fraction of potential gpp (no units)

     real(r8) , pointer :: fpi_p_vr                             (:,:)  => null()  ! col fraction of potential immobilization (no units)
     real(r8) , pointer :: fpi_p                                (:)    => null()  ! col fraction of potential immobilization (no units)
     real(r8),  pointer :: fpg_p                                (:)    => null()  ! col fraction of potential gpp (no units)
     ...
     real(r8), pointer :: externaln_to_decomp_npools            (:,:,:)  => null() ! col (gN/m3/s) net N fluxes associated with litter/som-adding/removal to decomp pools
     real(r8), pointer :: externalp_to_decomp_ppools            (:,:,:)  => null() ! col (gP/m3/s) net P fluxes associated with litter/som-adding/removal to decomp pools
     real(r8), pointer :: decomp_k_pools                        (:)      => null() ! rate constant for each decomposition pool (1./sec)
     real(r8), pointer :: sitefactor_kd_vr                      (:,:)    => null() ! a site factor for adjusting rate constant of all decomposition pools (-) (c,j)
     real(r8), pointer :: adfactor_kd_pools                     (:)      => null() ! a speed-up factor for adjusting rate constant of individual decomposition pool (-) (k)
     
     ! bgc rates/fluxes (previous time-step) to nh4 / no3
     real(r8), pointer :: externaln_to_nh4                      (:,:)  => null()  ! col (gN/m3/s) net N fluxes to nh4 pool: deposition + fertilization + supplement + nfix + soyfixn
     real(r8), pointer :: externaln_to_no3                      (:,:)  => null()  ! col (gN/m3/s) net N fluxes to no3 pool: deposition + fertilization + supplement
     real(r8), pointer :: externaln_to_sminn                    (:,:)  => null()  ! col (gN/m3/s) net N fluxes to sminn pool: deposition + fertilization + supplement + nfix + soyfixn
     real(r8), pointer :: smin_no3_leached_vr                   (:,:)  => null()  ! col vertically-resolved soil mineral NO3 loss to leaching (gN/m3/s)
     real(r8), pointer :: smin_no3_runoff_vr                    (:,:)  => null()  ! col vertically-resolved rate of mineral NO3 loss with runoff (gN/m3/s)

     ! bgc rates/fluxes (previous time-step) to mineral P
     real(r8), pointer :: externalp_to_primp                    (:,:)  => null()  ! pdep_to_sminp                         (:)  => null()    ! col atmospheric P deposition to soil mineral P (gP/m2/s)
     real(r8), pointer :: externalp_to_labilep                  (:,:)  => null()  ! fert_p_to_sminp                       (:)  => null()    ! col fertilizer P to soil mineral P (gP/m2/s)
     real(r8), pointer :: externalp_to_solutionp                (:,:)  => null()  ! supplement_to_sminp_vr                (:,:)  => null()  ! col vertically-resolved supplemental P supply (gP/m3/s)
     real(r8), pointer :: sminp_leached_vr                      (:,:)  => null()  ! col vertically-resolved soil mineral P pool loss to leaching (gP/m3/s)
     real(r8), pointer :: sminp_net_transport_vr                (:,:)  => null()  ! sminp_leached_vr                      (:,:)  => null()col net sminp transport associated with runoff/leaching (gN/m3/s)

     ! gases:
     real(r8), pointer :: f_ngas_decomp_vr                      (:,:)  => null()  ! col vertically-resolved N emission from excess mineral N pool due to mineralization (gN/m3/s)
     real(r8), pointer :: f_ngas_nitri_vr                       (:,:)  => null()  ! col vertically-resolved N emission from nitrification (gN/m3/s)
     real(r8), pointer :: f_ngas_denit_vr                       (:,:)  => null()  ! col vertically-resolved N emission from denitrification (gN/m3/s)

     ! aq. phases:
     real(r8), pointer :: no3_net_transport_vr                  (:,:)  => null()  ! col net NO3 transport associated with runoff/leaching (gN/m3/s) - also store PF's N transport inc. diffusion at current time-step
     real(r8), pointer :: nh4_net_transport_vr                  (:,:)  => null()  ! col net NH4 transport associated with runoff/leaching (gN/m3/s) - also store PF's N transport inc. diffusion at current time-step
     ...
     real(r8), pointer :: soil_begcb                            (:)  => null()    ! soil organic carbon mass, beginning of time step (gC/m**2)
     real(r8), pointer :: soil_begnb                            (:)  => null()    ! soil nitrogen mass, beginning of time step (gN/m**2)
     real(r8), pointer :: soil_begnb_org                        (:)  => null()    ! soil organic nitrogen mass, beginning of time step (gN/m**2)
     real(r8), pointer :: soil_begnb_min                        (:)  => null()    ! soil mineral nitrogen mass, beginning of time step (gN/m**2) = no3 + nh4 + nh4sorb
...
!-------------------------------------------------------------------------------------------------
  ...
  subroutine InitAllocate(this, bounds)
    ...
    allocate(this%o2stress_unsat            (begc:endc,1:nlevgrnd))     ; this%o2stress_unsat           (:,:) = spval
    allocate(this%o2stress_sat              (begc:endc,1:nlevgrnd))     ; this%o2stress_sat             (:,:) = spval
    allocate(this%conc_o2_sat               (begc:endc,1:nlevgrnd))     ; this%conc_o2_sat              (:,:) = spval
    allocate(this%conc_o2_unsat             (begc:endc,1:nlevgrnd))     ; this%conc_o2_unsat            (:,:) = spval
    allocate(this%o2_decomp_depth_sat       (begc:endc,1:nlevgrnd))     ; this%o2_decomp_depth_sat      (:,:) = spval
    allocate(this%o2_decomp_depth_unsat     (begc:endc,1:nlevgrnd))     ; this%o2_decomp_depth_unsat    (:,:) = spval

    ! cnstate_vars:
    allocate(this%rf_decomp_cascade(begc:endc,1:nlevdecomp_full,1:ndecomp_cascade_transitions));
    this%rf_decomp_cascade(:,:,:) = spval

    allocate(this%pathfrac_decomp_cascade(begc:endc,1:nlevdecomp_full,1:ndecomp_cascade_transitions));
    this%pathfrac_decomp_cascade(:,:,:) = spval
    ...
    ! fpi, fpg
    allocate(this%fpi_vr                (begc:endc,1:nlevdecomp_full))  ; this%fpi_vr                   (:,:) = spval
    allocate(this%fpi                   (begc:endc))                    ; this%fpi                      (:)   = spval
    allocate(this%fpg                   (begc:endc))                    ; this%fpg                      (:)   = spval
    allocate(this%fpi_p_vr              (begc:endc,1:nlevdecomp_full))  ; this%fpi_p_vr                 (:,:) = spval
    allocate(this%fpi_p                 (begc:endc))                    ; this%fpi_p                    (:)   = spval
    allocate(this%fpg_p                 (begc:endc))                    ; this%fpg_p                    (:)   = spval
